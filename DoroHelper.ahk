...
Login()
{
    ...
    while !UserCheckColor(stdCkptX, stdCkptY, desiredColor, scrRatio) {
        ...
        ;新版下载
        if UserCheckColor([1951, 1848], [1328, 1344], ["0x00ADFB", "0xF5F5F3"], scrRatio) {
            UserClick(2175, 1392, scrRatio)
            Sleep sleepTime
        }
...
ClickOnDoro(*)
{
    ...
    if isOverclockRefreshed == 1 
        MsgBox "模拟室超频已经刷新了，快去看看吧"

    MsgBox "Doro完成任务！记得异拦再扫荡两次，竞猜、签到等活动清完哦~" CompanyTowerInfo()
...
;创建gui
doroGui := Gui(, "DoroHelper " currentVersion)
doroGui.Opt("+Resize")
